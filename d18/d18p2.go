...
func main() {
	...
	// Simulate the bytes falling onto the memory space
	for i := 0; i < numBytesToSimulate && i < len(bytePositions); i++ {
		grid := simulateFallingBytes(bytePositions[:i+1])
		steps, _ := findMinSteps(grid)
		if steps == -1 {
			fmt.Printf("The first byte that prevents the exit from being reachable is at %d,%d (iteration %d)\n", bytePositions[i].x, bytePositions[i].y, i+1)
			continue
		}
	}
