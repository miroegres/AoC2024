...
func main() {
	...
	shortestPath := bfs(racetrack, start, end, cache)
	fmt.Println("Shortest path length without cheating:", shortestPath)

	bestCheatsCount, cheatsMap := findBestCheats(racetrack, start, end, cheatTime, cache)
...
func bfs(racetrack [][]rune, start, end Point, cache map[Point]int) int {
...
func bfsWithCheat(racetrack [][]rune, start, end, cheatStart, cheatEnd Point, cache map[Point]int) int {
	queue := []Point{start}
	visited := make(map[Point]bool)
	visited[start] = true
	distance := make(map[Point]int)
...
func findBestCheats(racetrack [][]rune, start, end Point, cheatTime int, cache map[Point]int) (int, [][]rune) {
	originalPathLength := bfs(racetrack, start, end, cache)
	cheatsCount := 0
		...
		for x := range racetrack[0] {
			...
			for _, dir := range directions {
				...
				if isValidMove(racetrack, cheatEnd2) {
					pathLengthWithCheat1 := bfsWithCheat(racetrack, start, end, cheatStart, cheatEnd1, cache)
					pathLengthWithCheat2 := bfsWithCheat(racetrack, start, end, cheatStart, cheatEnd2, cache)
